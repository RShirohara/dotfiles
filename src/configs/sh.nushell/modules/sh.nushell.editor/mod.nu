...
export def apply-autopair-left [key: string]: nothing -> nothing {
  ...
  let buffer_right: string = (
    $buffer
    | str substring --grapheme-clusters $cursor_pos..
  )
  let key_pair_right: string = (
    $autopair_pairs
    | find --no-highlight $key
    | get 0
    | str substring --grapheme-clusters 1..1
  )
...
export def apply-autopair-right [key: string]: nothing -> nothing {
  ...
  let buffer_next_char: string = (
    $buffer
    | str substring --grapheme-clusters $cursor_pos..
    | str substring --grapheme-clusters 0..0
  )
...
export def apply-autopair-same [key: string]: nothing -> nothing {
  ...
  let buffer_left_key_chars_mod: int = (
    match $cursor_pos {
      ...
      1.. => {
        $buffer
        | str substring --grapheme-clusters ..($cursor_pos - 1)
      }
    }
    | split chars
    | where $it == $key
...
export def apply-autopair-backspace []: nothing -> nothing {
  ...
  let buffer_right: string = ($buffer | str substring --grapheme-clusters $cursor_pos..)
  let buffer_prev_char: string = (
    $buffer_left
    | str substring --grapheme-clusters (
      (($buffer_left | str length) - 1)..(($buffer_left | str length) - 1)
    )
  )
  let buffer_next_char: string = ($buffer_right | str substring --grapheme-clusters 0..0)

  let updated_buffer = if (
    ($autopair_pairs | where $it == ($buffer_prev_char + $buffer_next_char) | length) >= 1
  ) {
    (
      ($buffer_left | str substring --grapheme-clusters ..-2)
      + ($buffer_right | str substring --grapheme-clusters 1..)
    )
  } else {
    ($buffer_left | str substring --grapheme-clusters ..-2) + $buffer_right
  }
  ...
  $env.config.keybindings ++= [
    {
      ...
      event: [
        { send: "ExecuteHostCommand", cmd: "sh.nushell.editor apply-autopair-left '('" }
      ]
    },
    {
      ...
      mode: ["emacs", "vi_insert"]
      event: [
        { send: "ExecuteHostCommand", cmd: "sh.nushell.editor apply-autopair-right ')'" }
      ]
    },
    {
    ...
    },
    {
      ...
      mode: ["emacs", "vi_insert"]
      event: [
        { send: "ExecuteHostCommand", cmd: "sh.nushell.editor apply-autopair-same '\"'" }
      ]
    },
    {
    ...
    },
    {
      ...
      mode: ["emacs", "vi_insert"]
      event: [
        { send: "ExecuteHostCommand", cmd: "sh.nushell.editor apply-autopair-backspace" }
      ]
    }
  ]
}
